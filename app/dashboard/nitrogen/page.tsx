...
export default function NitrogenPage() {
  ...
  const [soilForm, setSoilForm] = useState({
    ...
    depth_cm: '60', no3_n_kg_ha: '', nh4_n_kg_ha: '', notes: '',
  })

  const [appForm, setAppForm] = useState({
    applied_at: format(new Date(), 'yyyy-MM-dd'),
    product: 'Urea', rate_kg_ha: '', n_kg_ha: '', n_percent: '46',
    method: 'broadcast', notes: '',
  })

  // Load overview
  useEffect(() => {
    fetch('/api/nitrogen/overview')
      .then(r => r.json())
      .then(d => {
  ...
  async function submitSoilTest(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/nitrogen/${selectedStation}/events`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'soil_test', ...soilForm }) })
  ...
  async function submitApplication(e: React.FormEvent) {
    e.preventDefault(); setSaving(true); setMessage(null)
    try {
      const res = await fetch(`/api/nitrogen/${selectedStation}/events`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'application', ...appForm }) })
      const d = await res.json()
      if (!res.ok) throw new Error(d.error)
  ...
  const statusStyles: Record<string, { card: string; badge: string; text: string }> = {
    ...
    MARGINAL:   { card: 'bg-amber-900/30 border-amber-700/50',   badge: 'text-amber-400 border-amber-700/50 bg-amber-900/30',   text: 'text-amber-400' },
    DEFICIENT:  { card: 'bg-red-900/30 border-red-700/50',       badge: 'text-red-400 border-red-700/50 bg-red-900/30',         text: 'text-red-400' },
  }
  const styles = statusStyles[currentStatus] ?? statusStyles.SUFFICIENT

  const chartData = data?.decile_chart?.map((row: any, i: number) => ({ ...row, label: DECILE_SHORT[i] })) ?? []
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div>
        ...
        <p className="text-sm text-stone-500 mt-0.5">Farm N overview and paddock detail</p>
      </div>
      ...
      <div className="card p-5">
        ...
        {overviewLoading ? (
          <div className="flex items-center justify-center h-32">
            <div className="w-6 h-6 border-2 border-field-600 border-t-transparent rounded-full animate-spin" />
          </div>
        ) : overview.length === 0 ? (
        ...
        {totalNGap > 0 && (
          <div className="mt-4 pt-4 border-t border-[#344a20] text-xs text-stone-500">
            Total N deficit across farm: <span className="text-amber-400 font-mono font-bold">{Math.round(totalNGap)} kg N</span>
            {' '}({Math.round(totalNGap / 0.46)} kg urea equivalent)
          </div>
        )}
      </div>
        ...
        <div className="card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
            <h2 className="font-semibold text-stone-200">
              {overview.find(s => s.station_id === selectedStation)?.paddock_name || selectedStation}
            </h2>
            <div className="flex gap-2">
              <button onClick={() => { setShowSoilForm(s => !s); setShowAppForm(false) }}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${showSoilForm ? 'bg-field-800 border-field-600 text-field-200' : 'border-[#344a20] text-stone-400 hover:text-stone-200'}`}>
                + Soil Test
              </button>
          ...
          {showSoilForm && (
            <form onSubmit={submitSoilTest} className="mb-4 border border-[#344a20] rounded-lg p-4 space-y-3">
              ...
              <div><label className={labelCls}>Notes</label><input type="text" className={inputCls} value={soilForm.notes} onChange={e => setSoilForm(p => ({ ...p, notes: e.target.value }))} /></div>
              <div className="flex gap-2">
          ...
          {showAppForm && (
            <form onSubmit={submitApplication} className="mb-4 border border-[#344a20] rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <div><label className={labelCls}>Product</label><select className={inputCls} value={appForm.product} onChange={e => handleProductChange(e.target.value)}>{N_PRODUCTS.map(p => <option key={p.name} value={p.name}>{p.name} ({p.n_percent}% N)</option>)}</select></div>
                ...
                <div><label className={labelCls}>Rate (kg/ha)</label><input type="number" step="0.5" className={inputCls} placeholder="100" required value={appForm.rate_kg_ha} onChange={e => handleRateChange(e.target.value)} /></div>
        ...
        <>
          ...
          <div className={`card p-5 border ${styles.card}`}>
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              <span className={`text-sm font-mono font-bold px-3 py-1.5 rounded-lg border shrink-0 ${styles.badge}`}>{balance.status}</span>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3">
              <div className="bg-[#1a2310] rounded-lg p-3 border border-[#344a20]"><div className="text-xs text-stone-500 mb-1">Soil N (last test)</div><div className="font-mono text-lg font-bold text-field-300">{balance.soil_n} <span className="text-xs text-stone-500 font-normal">kg/ha</span></div></div>
              <div className="bg-[#1a2310] rounded-lg p-3 border border-[#344a20]"><div className="text-xs text-stone-500 mb-1">Applied this season</div><div className="font-mono text-lg font-bold text-blue-400">{balance.applied_n} <span className="text-xs text-stone-500 font-normal">kg N/ha</span></div></div>
              <div className="bg-[#1a2310] rounded-lg p-3 border border-[#344a20]"><div className="text-xs text-stone-500 mb-1">Season rainfall</div><div className="font-mono text-lg font-bold text-sky-400">{balance.season_rain_mm} <span className="text-xs text-stone-500 font-normal">mm</span></div></div>
              <div className={`rounded-lg p-3 border ${balance.leaching_risk === 'HIGH' ? 'bg-red-900/20 border-red-800/40' : balance.leaching_risk === 'MODERATE' ? 'bg-amber-900/20 border-amber-800/40' : 'bg-[#1a2310] border-[#344a20]'}`}>
                <div className="text-xs text-stone-500 mb-1">Leaching <span className={`font-bold ${balance.leaching_risk === 'HIGH' ? 'text-red-400' : balance.leaching_risk === 'MODERATE' ? 'text-amber-400' : 'text-stone-500'}`}>({balance.leaching_risk})</span></div>
              ...
              <div className="bg-[#1a2310] rounded-lg p-3 border border-[#344a20]"><div className="text-xs text-stone-500 mb-1">Crop uptake ({balance.gdd_percent}% GDD)</div><div className="font-mono text-lg font-bold text-amber-400">-{balance.crop_uptake} <span className="text-xs text-stone-500 font-normal">kg/ha</span></div></div>
              <div className={`rounded-lg p-3 border ${styles.card}`}><div className="text-xs mb-1 opacity-75">Available N</div><div className={`font-mono text-lg font-bold ${styles.text}`}>{balance.available_n} <span className="text-xs font-normal opacity-75">kg N/ha</span></div></div>
            </div>
            <div className="text-xs text-stone-600">Leaching adjusted for {data.station.soil_type ?? 'loam'} soil · Uptake from GDD · Consult your agronomist for fertiliser decisions</div>
          </div>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={chartData} margin={{ top: 20, right: 8, bottom: 0, left: -10 }} barGap={2}>
                  ...
                  <Bar dataKey="yield_with_current_n" fill="#4ade80" radius={[3, 3, 0, 0]} maxBarSize={60}>
                    <LabelList dataKey="n_topup" position="top" formatter={(val: any) => val > 0 ? `+${val}kg` : '✓'} style={{ fill: '#a3a3a3', fontSize: '9px' }} />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div className="mt-4 overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-[#344a20]"><th className="px-3 py-2 text-left text-stone-500 uppercase tracking-wider">Rainfall</th><th className="px-3 py-2 text-right text-stone-500 uppercase tracking-wider">Potential</th><th className="px-3 py-2 text-right text-stone-500 uppercase tracking-wider">With current N</th><th className="px-3 py-2 text-right text-stone-500 uppercase tracking-wider">N top-up needed</th></tr></thead>
                  <tbody>
                    {chartData.map((row: any, i: number) => (
                      <tr key={i} className={`border-b border-[#2a3c18] ${i === 2 ? 'bg-field-900/20' : i % 2 === 0 ? '' : 'bg-[#1e2812]/40'}`}>
                        <td className={`px-3 py-2 ${i === 2 ? 'text-field-300 font-medium' : 'text-stone-300'}`}>{row.label}{i === 2 && <span className="ml-1 text-[10px] text-field-500">← most likely</span>}</td>
                        ...
                        <td className={`px-3 py-2 text-right font-mono ${row.n_topup > 0 ? 'text-amber-400' : 'text-emerald-500'}`}>{row.n_topup > 0 ? `${row.n_topup} kg N/ha` : '✓ Sufficient'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {!data.soil_tests?.length && !data.applications?.length && (
            <div className="card p-12 text-center">
              ...
              <div className="text-stone-400 font-medium mb-2">No nitrogen data yet for this paddock</div>
